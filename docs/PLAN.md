...
| 0007 | Geocoding | Google Places API (Text Search + Details) with Nominatim fallback | Resolving Thai-script place names needs Google's index | Mapbox Geocoding if Places pricing hurts |
| 0008 | Maps (UI) | MapLibre GL with a styled vector basemap | Free, styleable to the brand palette, no Google Maps branding in the UI | Leaflet if MapLibre is overkill (Aswin has Leaflet experience from the ARG) |
...
- **P2.3** **resolve stage**: generic — take pending Mentions, call `pack.resolve`, cache by `(domainId, normalised key)`, mark `unresolvable` after N attempts. Then the travel resolver: Google Places Text Search biased to city bbox, confidence score, Nominatim fallback. Unresolved mentions still produce a `Place` with `geo: null`, flagged.
- **P2.4** **dedup stage**: generic — walk `pack.dedupKeys(entity)` strongest-first, ask `repo.findByKeys`, call `repo.merge` on a hit. Merge preserves all Evidence. Then travel's keys: googlePlaceId (exact), normalised name + 150m radius (geo), embedding similarity above threshold (embedding).
...
| Geocoding | ~$5 | 800 Places calls | **The wildcard.** Check Google's current rate and whether the monthly free credit applies *before* Phase 2 starts. This is the one meter that could blow the ceiling on its own. |

About $4 stays unallocated as headroom.
...
- **Cache geocodes permanently**, keyed by normalised name. Coordinates do not change. Nominatim first; Google only when it fails.
...
- **The card exposure that remains is Google, not Solari.** Solari stops when the credit is gone.
  OpenRouter is prepaid, so it also stops. **Google Places bills a card by default once its free
  credit is exhausted** — it is the one meter here that can quietly become a charge. Before Phase 2:
  set a hard daily quota cap on the Places API in the Google Cloud console (a quota cap fails
  requests; a budget *alert* only sends mail), confirm the current rate, and re-derive the 800-call
  ceiling from it. Until that is done, resolution runs on Nominatim only.
